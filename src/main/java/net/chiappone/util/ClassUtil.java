...
public class ClassUtil {
	...
	static {
		...
		collectionTypes.addAll(
				Arrays.asList( "Collection", "BeanContext", "BeanContextServices", "BlockingQueue",
						"List", "Queue", "Set", "SortedSet" ) );

		// Collection Implementations
		collectionTypes.addAll(
				Arrays.asList( "AbstractCollection", "AbstractList", "AbstractQueue",
						"AbstractSequentialList", "AbstractSet", "ArrayBlockingQueue", "ArrayList",
						"AttributeList", "BeanContextServicesSupport", "ConcurrentLinkedQueue",
						"CopyOnWriteArrayList", "CopyOnWriteArraySet", "DelayQueue", "EnumSet",
						"HashSet", "JobStateReasons", "LinkedBlockingQueue", "LinkedHashSet",
						"LinkedList", "PriorityBlockingQueue", "PriorityQueue", "RoleList",
						"RoleUnresolvedList", "Stack", "SynchronousQueue", "TreeSet", "Vector" ) );

		// Primitives
		primitiveTypes.addAll(
				Arrays.asList( "byte", "char", "character", "int", "integer", "double", "float",
	...
	public static String extractParameterization( String parameterizedClass ) {
		...
		return parameterizedClass.substring( parameterizedClass.indexOf( "<" ) + 1,
				parameterizedClass.lastIndexOf( ">" ) );
